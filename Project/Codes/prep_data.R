...
for (i in 1:length(file_list)){
  ...
  temp <- read.csv(paste(folder, file_list[i], sep=''), stringsAsFactors = F)
  ticker <- temp[1, "Name"]
  temp <- xts(temp[, "volume"], order.by = as.Date(temp[, "date"]))
  colnames(temp) <- ticker
  volume_dat <- cbind(volume_dat, temp)
}
...
ncol(volume_dat)
nrow(volume_dat)
length(index(volume_dat))
volume_dat[1:100, ]

# Find which firms have missing data
na_count <- colSums(is.na(volume_dat))
str(na_count)
...
retain_firms <- setdiff(colnames(volume_dat), miss_val_firms)
length(retain_firms)

volume_dat <- volume_dat[, retain_firms]

dates <- as.character(as.Date(index(volume_dat)))
spvol_dat <- data.frame(cbind(dates, coredata(volume_dat)))
colnames(spvol_dat) <- c("date", colnames(volume_dat))
...
save(spvol_dat, file = "C:/local/sandp500/spvolume.RData")
write.csv(spvol_dat, file = "C:/local/sandp500/sp470_volume.csv", row.names = F)
